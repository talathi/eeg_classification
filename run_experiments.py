	...
	for k in range(4):
		if not gpu_under_use(3-k):
			...
			str_cmd='THEANO_FLAGS=\'device=gpu%d,lib.cnmem=0.8\' python eeg_classification --train --learning-rate 0.001 --cool --epochs 300 --window-size 80\
			 --save-dir ~/Python/Models/keras_models/rnneeg_models --memo rnn_eeg_model_case0_w80_300epochs_%d\
			>> ~/Python/Models/keras_models/conveeg_models/logfiles/rnn_eeg_model_case0_w80_300epochs_%d.log 2>&1&'%(k,count,count)
			print str_cmd
			os.system(str_cmd)
			time.sleep(10)
			count+=1
